...
class ActiveMasterManager extends ZooKeeperListener {
  ...
  boolean blockUntilBecomingActiveMaster(MonitoredTask startupStatus,
    ...
    while (true) {
      ...
      try {
        String backupZNode = ZKUtil.joinZNode(//  '/hbase/backup-masters/{ServerName.toString()}' 备份节点
          this.watcher.backupMasterAddressesZNode, this.sn.toString());
        if (ZKUtil.createEphemeralNodeAndWatch(this.watcher,
          this.watcher.masterAddressZNode, this.sn.getVersionedBytes())) {//创建临时节点/hbase/master 节点值为version+ServerName 若创建成功则删除备份节点否则创建备份节点
          // If we were a backup master before, delete our ZNode from the backup
          // master directory since we are the active now
          LOG.info("Deleting ZNode for " + backupZNode +
            " from backup master directory");
          ZKUtil.deleteNodeFailSilent(this.watcher, backupZNode);//删除备份节点
        ...
        if (bytes == null) {//master节点挂掉了
          ...
          if (ServerName.isSameHostnameAndPort(currentMaster, this.sn)) {//查看自己是否是master
