...
Server {
  ...
  throws IOException, KeeperException, InterruptedException {
    ...
    this.conf.setFloat(HConstants.HFILE_BLOCK_CACHE_SIZE_KEY, 0.0f);//hfile.block.cache.size 禁止master上的block cache
    // Set how many times to retry talking to another server over HConnection.
    //设置通过HConnection与其他服务器通信重试次数
    HConnectionManager.setServerSideHConnectionRetries(this.conf, LOG);//hbase.client.retries.number
    // Server to handle client requests.
    String hostname = Strings.domainNamePointerToHostName(DNS.getDefaultHost(
      conf.get("hbase.master.dns.interface", "default"),
      conf.get("hbase.master.dns.nameserver", "default")));
    int port = conf.getInt(HConstants.MASTER_PORT, HConstants.DEFAULT_MASTER_PORT);//hbase.master.port master监听端口号 默认为60000 
    ...
    //设置当前线程名称 master-this.serverName.toStirng()
    ...
    int sleepTime = this.conf.getInt(HConstants.HEALTH_CHORE_WAKE_FREQ,//hbase.node.health.script.frequency  线程wake频率 默认值为10 * 1000
      HConstants.DEFAULT_THREAD_WAKE_FREQUENCY);
    if (isHealthCheckerConfigured()) {//是否需要进行健康监测
  ...
  throws InterruptedException {
    ...
    if (!c.getBoolean(HConstants.MASTER_TYPE_BACKUP,//hbase.master.backup 默认为false
  ...
  throws InterruptedException {
    ...
    stallIfBackupMaster(this.conf, this.activeMasterManager);//检查hbase.master.backup 查看自己是否backup机器
  ...
  private void initializeZKBasedSystemTrackers() throws IOException,
	...
	//包含'/hbase/root-region-server' 和 '/hbase/unassigned/{xxxx}'节点
    this.catalogTracker = new CatalogTracker(this.zooKeeper, this.conf, this);
    this.catalogTracker.start();

    //loaderBalancer 负责region在regionserver之间的移动
    this.balancer = LoadBalancerFactory.getLoadBalancer(conf);
    //AssignmentManager负责管理和分配region 同时也会接受zookeeper上关于region的event 根据event来完成region的上下线、关闭打开灯工作
    this.assignmentManager = new AssignmentManager(this, serverManager,
        this.catalogTracker, this.balancer, this.executorService);
    zooKeeper.registerListenerFirst(assignmentManager);

    //监控节点'/hbase/rs' 通过zookeeper的event来跟踪onlineregion servers 若有rs下线 删除serverManager中对应的onlineregions
    ...
    //ClusterStatusTracker 监控节点'/hbase/shutdown' 维护集群状态
  ...
  throws IOException, InterruptedException, KeeperException {
    ...
    //创建MasterFileSystem封装了一些常用文件系统操作 包括splitlog file、删除region目录、删除table目录、删除cf目录、检查文件系统状态等。
    ...
    if (!masterRecovery) {//如果不是备份master
      this.executorService = new ExecutorService(getServerName().toString());
      //管理regionserver信息,维护onlineregion server和deadregion server列表 处理regionserver的startups、shutdowns、deaths,同时维护着每个regionserver rpc stub
    ...
    if (!masterRecovery) {//若不是备份master 启动AssignmentManager
      this.assignmentManager.startTimeOutMonitor();
    }
    // TODO: Should do this in background rather than block master startup
    status.setStatus("Splitting logs after master startup");
    //master startup后开始拆分HLog
    splitLogAfterStartup(this.fileSystemManager);

    // Make sure root and meta assigned before proceeding.
    //检查是否分配了-ROOT-表 若没有 则调用assignmentManager.assignRoot分配root表 并激活该表
    ...
    if (!masterRecovery) {
      ...
      startCatalogJanitorChore();//周期性扫描.META.表上未使用的region并回收
      registerMBean();
    }
  ...
  private boolean isHealthCheckerConfigured() {
    String healthScriptLocation = this.conf.get(HConstants.HEALTH_SCRIPT_LOC);//hbase.node.health.script.location
